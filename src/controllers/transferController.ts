...
import { IUser } import { Request, Response, NextFunction } from "express";
import { validationResult } from "express-validator";
import Transfer from "../models/Transfer";
import {
	successResponse,
...
export const getTransfers = async (
  ...
  try {
    ...
    const pipeline: any[] = [
      ...
      // Lookup and unwind complaint info
      {
        $lookup: {
          from: "complaints",
      ...
      // Project only needed fields
      {
        $project: {
          ...
          acceptedAt: 1,
          inTransitAt: 1,
          // User fields - only specific ones
          "user.name": 1,
          "user.email": 1,
          "user.phone": 1,
          "user.informationPreference": 1,
          // Worker fields - only specific ones
          "worker.name": 1,
          "worker.email": 1,
          "worker.isAvailable": 1,
          "worker.status": 1,
          "worker.phone": 1,
          // Complaint fields
          complaint: 1,
    ...
    const totalResult = await Transfer.aggregate(totalPipeline);
